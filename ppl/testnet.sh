...
create_tmux_space(){
    ...
    for i in $(seq 2 $1)
    do
        tmux new-window -t $TMUXSN -n NS$i ip netns exec NS$i bash
    done
    ...
    #tmux send-keys -t NS$(($NAMESPACES-1)) "./color_changer.py &" C-m 
...
sudo ip link set br-ctrl0 master br1
sudo ip link set br-ctrl1 master br1
sudo ip link set br-ctrl2 master br1
